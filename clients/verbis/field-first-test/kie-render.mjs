// Render a render-jobs JSON via KIE (GPT Image 2). Uploads local refs through KIE's
// file API (base64), then createTask on gpt-image-2-image-to-image (or text-to-image
// when a job has no refs), polls, downloads.
//   node kie-render.mjs <job-file.json>
...
const VERBIS_DIR = path.resolve(__dirname, '..'); // refs in job files are relative to clients/verbis
...
const outDir = path.resolve(VERBIS_DIR, spec.outDir ?? 'renders');
fs.mkdirSync(outDir, { recursive: true });

async function uploadRef(rel) {
  const abs = path.resolve(VERBIS_DIR, rel);
  ...
  const r = await fetch('https://kieai.redpandaai.co/api/file-base64-upload', {
    ...
    body: JSON.stringify({
      ...
      uploadPath: 'verbis-refs', fileName: path.basename(abs),
    }),
  });
