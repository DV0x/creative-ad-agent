/**
 * Verbis render helper — GPT Image 2 (OpenAI, via fal.ai).  [CommonJS]
 *
 ...
 *   - refs: local paths (relative to clients/verbis) → uploaded to fal → edit endpoint.
...
(async () => {
  ...
  console.log(`\n${ok}/${spec.jobs.length} rendered → clients/verbis/${spec.outDir || 'renders'}/`);
})();
