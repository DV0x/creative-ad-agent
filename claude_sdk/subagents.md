...
<Note>
  **Nested subagents ARE supported** — as of Claude Code **v2.1.172** (June 10 2026). *(This local doc was
  corrected 2026-06-30; it previously said "Subagents cannot spawn their own subagents," which is stale and
  misled two grounding agents.)* To let a subagent spawn its own subagents, include `Agent` in its `tools`;
  to prevent it, omit `Agent` or add it to `disallowedTools`. Hard depth cap: **5 levels**. Our SDK `0.3.195`
  ships CLI `2.1.195`, so nesting works here. Authoritative: https://code.claude.com/docs/en/agent-sdk/subagents
</Note>
